...
若新 DSH 因任何原因启动失败(事件 A 就是实例),助手会记录端口等待失败,**不会回滚**,
...
| 层 | 行为 |
|---|---|
| **助手内重试** | 启动失败后最多重试 3 次(每次等 `startTimeoutMs`、间隔 3 秒)。**仅当上一个子进程确实已退出**才重试 —— 活着的子进程仍可能稍后绑上端口,此时另起一个就是两个 DSH 抢同一端口 |
| **延时兜底** | 三次都失败后,再安排**一次** 60 秒后的尝试,由一个 `detached` 的子进程执行(它能活过助手);该子进程带 `--second-chance`,**不允许再安排下一次**,所以不会变成循环 |
| **端口被占而跳过** | 只在该次是**重启**路径时才安排兜底。登录时端口被占通常意味着别人已经在服务 DSH,去抢是不对的;重启时端口被占才是"我们自己的实例刚死、有东西残留",值得再试一次 |

**仍然没有做的**:回滚到重启前的状态 —— 那本就不可能,旧进程已经退出。所以兜底的语义是
**再试一次把它拉起来**,不是撤销。两份 README 都已写明"失败后先等 1~2 分钟,插件会自己兜底"。
...
socket 有机会消失;持续应答者仍会被判为外来服务并拒绝启动(安全底线不变)。日志文案改成如实
描述观测(「still answering after N probes」),不再下结论(「already running」)。

**已验证**:① 新增两条单元测试(重试后启动 / 持续占用仍拒绝,全套 125/125 通过);
② **真实 socket** 双向复核 —— 端口被占 1.6 秒后自行释放 → 重试到 1.6s 后成功启动;
端口持续占用 → 13 次探测 / 3.0 秒后拒绝启动。

**未修(放大器)**:`index.js:449` 在**结果尚不可知**时就回 `202 {accepted:true}`,而帮手放弃
启动时**没有任何面向用户的反馈** —— 于是这类失败仍然是"静默变砖":用户只看到转圈。建议把结局
(已启动 / 已跳过 / 失败)写成卡片可读的状态。

### 发现 F9 —— 宿主没有控制台 ⇒ 每条命令弹一个 Windows Terminal 窗口(已定位,未修)
...
**一行修法已实测否决。** 去掉 `detached` 让宿主继承控制台,但 A/B 对照(`detached` 为唯一变量,
父进程同为 WMI 创建、`windowsHide` 保持常量)结果:

| 模式 | 子进程 | 观察 |
|---|---|---|
| `detached: true`(现行) | 496 | **存活**(>2.5 分钟) |
| `detached: false`(拟改) | 21196 | **10 秒内被杀** |

原因:**控制台随它的创建者进程销毁,挂在上面的进程被连坐杀掉**。凶手不是 job ——
`DETACHED_PROCESS` 并不逃出 job,而只有 attached 的那个死了。所以留着 `detached` 是必须的:
清掉它会把"重启"重新变成"关机"(`service.js` 中已就此加了注释)。

**方向(未做)**:让启动器**常驻**(`service.js start` 不再"确认端口就退出"),宿主 attached 于
它的隐藏控制台 —— 控制台不销毁、宿主也不会被杀;重启也能交给它,进而彻底不依赖 WMI。
属设计改动,单独一轮做。
