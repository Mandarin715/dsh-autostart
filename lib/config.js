...
export const PLUGIN_DEFAULTS = {
  ...
  dshPort: 3080,
  exitDelayMs: 800,
  // Unused since Task 7 deleted runRestart, its only reader. Kept so an existing config.json
  // that sets it keeps loading rather than failing validation.
  waitForExitMs: 30000,
...
/**
 * The supervisor's three state files.
 *
 * Derived from `path.dirname(configFilePath(dshHome))` on purpose, and not from a second
 * spelling of the directory: the supervisor computes all three as
 * `path.dirname(configPath)` from the `--config` path it is handed (see service.js
 * runSupervise), and the launcher is handed `configFilePath(dshHome)`. Deriving both sides
 * from that one expression is what keeps them from drifting. A drift here is silent: the
 * restart request lands in a file nobody watches and the restart simply never happens.
 */
export function supervisePidFile(dshHome) {
  return path.join(path.dirname(configFilePath(dshHome)), 'supervise.pid')
}

/** Where the host asks the supervisor to restart the DSH it just saw exit. */
export function restartRequestFile(dshHome) {
  return path.join(path.dirname(configFilePath(dshHome)), 'restart.request')
}

/** Where the host asks the supervisor to stop once its child is gone. */
export function superviseStopFile(dshHome) {
  return path.join(path.dirname(configFilePath(dshHome)), 'supervise.stop')
}
...
export function buildConfigFile(input) {
  ...
  return {
    ...
    command,
    // Recorded so a restart can re-assert the home: the helper is created across
    // the WMI boundary, which does not inherit the user's environment.
    dshHome: typeof dshHome === 'string' && dshHome !== '' ? dshHome : null,
...
export function parseConfigFile(text) {
  ...
  return {
    ...
    // null means "do not touch the environment": a file written before this field
    // existed has no home to re-assert, and inventing one would be worse.
    dshHome: typeof parsed.dshHome === 'string' && parsed.dshHome !== '' ? parsed.dshHome : null,
    ...plugin,
