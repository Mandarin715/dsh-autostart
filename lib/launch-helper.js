// Launch the resident supervisor OUTSIDE DSH's job object.
//
...
//
// What this file does now — and, deliberately, all it does — is hand the
// supervisor out of that job. The supervisor itself is what restarts DSH: it
// survives DSH's exit, learns of it from the child handle, and starts the
// replacement. Nothing here waits for a pid or starts DSH any more.
//
...
// survives DSH's exit and can supervise the replacement instance.
//
...
//     caller's environment (notably DSH_HOME) is NOT inherited. The supervisor
//     is therefore told its config path explicitly and must not re-derive it.
//  2. The provider host hands the child no standard handles, so the supervisor
...
/**
 * The command line that runs the supervisor:
 *   "<node>" "<service.js>" supervise --config "<config.json>" --takeover <n>
 *
 * `supervise` is the real entry point and `start` is its alias; `--takeover <n>`
 * names the DSH that is about to exit, so the supervisor waits for it and then
 * starts the replacement. Neither the supervisor nor this helper waits here: the
 * WMI call below only has to complete before DSH exits.
 *
 ...
 * @param input.takeoverPid - pid of the DSH the supervisor must take over from.
 * @param input.configPath - absolute path of config.json, required. The supervisor
 ...
 *   back to ~/.dsh — the supervisor would read the wrong config.json, exit 1, and
...
export function buildHelperCommandLine(input) {
  const { execPath, serviceJsPath, takeoverPid, configPath } = input ?? {}
  ...
  if (!Number.isInteger(takeoverPid) || takeoverPid <= 0) {
    throw new Error(`buildHelperCommandLine: takeoverPid must be a positive integer, got ${String(takeoverPid)}`)
  }
  if (typeof configPath !== 'string' || configPath === '') {
    throw new Error(
      'buildHelperCommandLine: configPath is required (the supervisor must not re-derive DSH_HOME)',
    )
  }
  if (!path.win32.isAbsolute(configPath)) {
    throw new Error(
      `buildHelperCommandLine: configPath must be absolute, got ${configPath} (a relative path would resolve against the supervisor's working directory)`,
    )
  }
  return `${quoteWinArg(execPath)} ${quoteWinArg(serviceJsPath)} supervise --config ${quoteWinArg(configPath)} --takeover ${takeoverPid}`
}
